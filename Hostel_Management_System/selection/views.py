    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            if request.user.student.room_id:
                ...
                print(request.user.student.room_id)
        ...
        student_gender = str(request.user.student.gender)
        student_course = str(request.user.student.course)
        if student_course is None:
                        return HttpResponse('<h3>No Course Selected </h3> <br> <a href = \'../edit\'' 
                        'style = "text-align: center; color: Red ;"> Update Profile </a>')
        student_room_type = request.user.student.course.room_type
        print(student_room_type)
        req_course = Course.objects.filter(code=student_course)
        hostel = Hostel.objects.filter(gender=student_gender, course=req_course[0])
        #print(student_gender, student_course, student_room_type)
        print('hostel',hostel)
        x = Room.objects.none()
        for i in range(len(hostel)):
            print(hostel[i].id)
        x = Room.objects.filter(hostel_id=hostel[0].id, vacant=True).order_by('hostel_id','no')
            #print(x)
            # x = x | a
